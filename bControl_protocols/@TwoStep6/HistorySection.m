...
switch action
    ...
    case 'next_trial'
       ...
       %     left_small_reward_visited = isfield(parsed_events.states, 'left_small_reward_state') && rows(parsed_events.states.left_unreward_state) > 0;
            ...
            one_step = ~(free_choice || force_left || force_right);
            ...
            elseif right_choice_1
                ...
                % Means that there never was a choice 2
                if sideChosen_2 == 'v'
                    sideChosen_2 = 'x'; 
                end
            end
    ...
    case 'update_plot'
        try
            ...
            % Hack away at the histories that might not exist, for backwards
            % compatibility
            if length(viol1_history) ~= length(reward_history)
                viol1_history.value = zeros(length(reward_history),1);
            end
            if length(trial_type_history) ~= length(reward_history)
                trial_type_history.value = char('f'*ones(length(reward_history),1));
            end
            ...
            for t = 1:4
                plot(ax, [0 n_done_trials + 1], [t t], ':', 'Color',[0.5 0.5 0.5]);
            end
            
            plot(ax, rewards_free, yvals(rewards_free),'g.',...
                'MarkerSize',15);
            plot(ax, unrewards_free, yvals(unrewards_free),'.',...
                'MarkerEdgeColor',[1, 0.66, 0],...
                'MarkerSize',15);
            plot(ax, rewards_forced, yvals(rewards_forced),'go',...
            ...
            plot(ax, violations, yvals(violations),'r.',...
                'MarkerSize',15);
            set(ax, 'Ylim', [0.5 4.5], 'XLim', [0 n_done_trials+1],'YTick', 1:4, 'YTickLabel', ylabels);
            hold(ax, 'off');
            ...
            set(ax2, 'Ylim', [0 1], 'XLim', [0 n_done_trials+1]);
            ...
            set(ax3, 'Ylim', [0 1], 'XLim', [0 n_done_trials+1]);
            hold(ax3,'off');
            
            
        catch error
            warning(['Problem with plot:',error]);
        end
