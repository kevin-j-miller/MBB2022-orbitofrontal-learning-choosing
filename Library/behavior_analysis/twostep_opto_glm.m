...
fldr = 'C:\Users\kevin\Documents\Princeton\code_for_twostep_physopto_paper\analysis_code\stan_models';
if ~exist('nBack','var')
    nBack = 3;
end    
...
for rat_i = 1:nRats
    ...
    standatas{rat_i}.commons = data_rat.trans_common;
...
for rat_i = 1:nRats
    ...
    fit = stan('file',[fldr '\twostep_glm_opto.stan'],...
    ...
    fit.block();
    fit_extracted = extract(fit);
   % fit_printed = print(fit);
    fits_extracted{rat_i} = fit_extracted;
end

for rat_i = 1:nRats
    p = fits_extracted{rat_i};
...
results.fits_extracted = fits_extracted;
results.nBack = nBack;
