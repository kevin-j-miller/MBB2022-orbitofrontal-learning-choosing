...
excluded_frac = mean(reg_results.bad_glm)
nRegs = size(reg_results.sse_leftout{1,1},1);

%% Compute CPD for all cells in each bin
for lock_i = 1:4
    sse_full = sum([reg_results.sse_full_all{lock_i, ~sse.bad_glm}],2)';

    temp = cell2mat(reshape(reg_results.sse_leftout(lock_i, ~sse.bad_glm),1,1,[]));
