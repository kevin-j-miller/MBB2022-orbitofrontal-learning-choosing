...
for rat_i = 1:length(ratdatas_all)
    ...
    fit = fit_stan_model(standata);
    fit_params(rat_i) = fit;

    % Compute the trial-by-trial model-derived values for this session,
    % using the parameters
    values = values_multiagent(fit.alphaMB, ...
        fit.alphaPersev,...
        fit.betaMB, ...
        fit.betaPersev,...
        fit.betaBonus, ...
...
% 6 - Control perseveration. 0 for none, 1 for one-back, 2 for habits
% 7 - Allow bias
...
save(fullfile(files_path, 'postprocessed_data', 'behavioral_model_fits'),...
    'fit_params', 'ratdatas', 'ratdata_ratnames', 'ratdata_conditions', 'xval_normLiks', 'xval_model_names');
